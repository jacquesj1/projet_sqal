...
class VL53L8CH_DataAnalyzer:
    def __init__(self,
                 ...
                 distance_defect_threshold_mm: float = 10.0,
                 critical_foreign_body_severity_threshold: float = 0.7,
                 defect_penalty_weight: float = 1.0):
        """
        ...
        distance_defect_threshold_mm: seuil en mm au-delà duquel une déviation de distance génère un défaut
        critical_foreign_body_severity_threshold: seuil de sévérité au-delà duquel un défaut foreign_body force REJECT
        """
        ...
        self.distance_defect_threshold_mm = float(distance_defect_threshold_mm)
        self.critical_foreign_body_severity_threshold = float(critical_foreign_body_severity_threshold)
        self.defect_penalty_weight = float(defect_penalty_weight)
    ...
    def _detect_surface_defects(self, distances: np.ndarray, heights: np.ndarray) -> List[Dict[str,Any]]:
        ...
        for i in range(res):
            for j in range(res):
                ...
                dev = d - median_d
                if abs(dev) > self.distance_defect_threshold_mm:
    ...
    def _compute_quality_score(self, stats: Dict, bins_analysis: Optional[Dict], refl_analysis: Dict, consistency: Dict, defects: List[Dict]) -> Tuple[float, Dict]:
        ...
        effective_defect_penalty = float(np.clip(defect_penalty * self.defect_penalty_weight, 0.0, 1.0))
        final_score = combined * (1.0 - effective_defect_penalty)
        ...
        breakdown = {
            ...
            "defect_penalty_weight": self.defect_penalty_weight,
            "effective_defect_penalty": effective_defect_penalty
        }
    ...
    def _determine_grade(self, quality_score: float, defects: List[Dict]) -> str:
        ...
        has_critical = any(
            d.get("type") == "foreign_body" and float(d.get("severity", 0.0)) > self.critical_foreign_body_severity_threshold
            for d in defects
        )
        if has_critical:
            return Grade.REJECT
