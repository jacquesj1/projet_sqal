...
class VL53L8CHRawData(BaseModel):
    ...
    amplitude_matrix: List[List[int]] = Field(
        ...,
        description="Matrice 8x8 amplitude signal retour",
        min_items=8,
        max_items=8
    )

    @validator('distance_matrix', 'reflectance_matrix', 'amplitude_matrix')
    ...
    @validator('reflectance_matrix', 'amplitude_matrix')
    def validate_0_255_range(cls, v):
        """Valide les valeurs 0-255"""
        for row in v:
            for val in row:
                if not (0 <= val <= 255):
                    raise ValueError(f"Valeur invalide: {val} (range: 0-255)")
        return v
