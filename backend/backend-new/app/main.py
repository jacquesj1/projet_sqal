...
from app.routers import euralis, sqal, consumer_feedback
...
# Application FastAPI
app = FastAPI(
    title="Système Gaveurs V2.1 - API IA & Blockchain + Euralis Multi-Sites + SQAL Qualité",
    description="API complète pour gavage intelligent avec régression symbolique, traçabilité blockchain, supervision multi-sites Euralis et contrôle qualité SQAL (VL53L8CH + AS7341)",
    version="2.1.0"
)
...
async def startup():
    ...
    # Initialiser le service SQAL avec le pool de connexions
    from app.services.sqal_service import sqal_service
    await sqal_service.init_pool(database_url)
    logger.info("✅ Service SQAL initialisé")

    # Initialiser le service Consumer Feedback
    from app.services.consumer_feedback_service import consumer_feedback_service
    await consumer_feedback_service.init_pool(database_url)
    logger.info("✅ Service Consumer Feedback initialisé")
...
async def shutdown():
    ...
    # Fermer le service SQAL
    from app.services.sqal_service import sqal_service
    await sqal_service.close_pool()
    logger.info("🔴 Service SQAL fermé")

    # Fermer le service Consumer Feedback
    from app.services.consumer_feedback_service import consumer_feedback_service
    await consumer_feedback_service.close_pool()
    logger.info("🔴 Service Consumer Feedback fermé")
