...
class ESP32_Simulator:
    ...
    def _select_quality_profile(self):
        ...
        """
        try:
            profile_name = getattr(self.config_loader, 'current_profile', None) or self.config_profile
            quality_config = (self.config or {}).get('profiles', {}).get(profile_name, {}).get('quality', {})

            if not isinstance(quality_config, dict) or not quality_config:
                raise KeyError(f"profiles.{profile_name}.quality")
    ...
    async def read_sensors(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # Ensure raw matrices include status/ambient for downstream pipeline (backend + UI)
            if isinstance(vl_data, dict):
                if vl_data.get('status_matrix') is None:
                    vl_data['status_matrix'] = [[5] * 8 for _ in range(8)]
                if vl_data.get('ambient_matrix') is None:
                    vl_data['ambient_matrix'] = [[0] * 8 for _ in range(8)]
            ...
            if vl_data and as_data:
                ...
                try:
                    raw_counts = (as_data or {}).get('raw_counts', {})
                    f1 = raw_counts.get('F1_violet')
                    f2 = raw_counts.get('F2_indigo')
                    f4 = raw_counts.get('F4_cyan')
                    f5 = raw_counts.get('F5_green')
                    f6 = raw_counts.get('F6_yellow')
                    f7 = raw_counts.get('F7_orange')
                    f8 = raw_counts.get('F8_red')
                    nir = raw_counts.get('NIR')
                    qc_sum = sum(v for v in raw_counts.values() if isinstance(v, (int, float)))

                    ratios = (as_analysis or {}).get('spectral_ratios', {})
                    qm = (as_analysis or {}).get('quality_metrics', {})
                    logger.info(
                        f"[{self.device_id}] AS7341 raw_counts sum={qc_sum:.0f} "
                        f"F1={f1} F2={f2} F4={f4} F5={f5} F6={f6} F7={f7} F8={f8} NIR={nir} | "
                        f"ratios vor={ratios.get('violet_orange_ratio')} discol={ratios.get('discoloration_index')} "
                        f"loi={ratios.get('lipid_oxidation_index')} fmi={ratios.get('freshness_meat_index')} ooi={ratios.get('oil_oxidation_index')} | "
                        f"qm freshness={qm.get('freshness_index')} fat={qm.get('fat_quality_index')} ox={qm.get('oxidation_index')} color={qm.get('color_uniformity')} score={qm.get('quality_score')}"
                    )
                except Exception:
                    logger.exception(f"[{self.device_id}] AS7341 debug log failed")
    ...
    def adapt_for_backend(self, sensor_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if vl_analysis:
            ...
            # Keep defects as list of dicts (type/pos/severity/...) so UI can place markers.
            # Do not coerce to strings.
            ...
            if vl_raw:
                ...
                vl_analysis['status_matrix'] = vl_raw.get('status_matrix', [[5]*8 for _ in range(8)])
        ...
        if as_analysis:
            ...
            if 'color_uniformity' not in as_analysis:
                as_analysis['color_uniformity'] = as_analysis.get('quality_metrics', {}).get('color_uniformity', 0.88)
            ...
            as_analysis['quality_metrics'] = {
                **as_analysis.get('quality_metrics', {}),
                ...
                'defects_detected': as_analysis.get('quality_metrics', {}).get('defects_detected', []),
            }

            # Keep defects as list of dicts (do not coerce to strings).
        ...
        if fusion:
            ...
            # Keep fusion defects as list of dicts when available.
    ...
    async def send_measurement(self, sensor_data: Dict[str, Any]):
        ...
        payload = {
            ...
            # Données brutes (utiles pour debug + composants UI)
            'vl53l8ch_raw': adapted_data.get('vl53l8ch_raw', {}),
            'as7341_raw': adapted_data.get('as7341_raw', {}),
        ...
        # Si OFFLINE → buffer local
            ...
            # Si on continue à produire des mesures mais qu'on est OFFLINE,
            # il faut forcer une tentative de reconnexion (sinon on reste bloqué à bufferiser).
            if self.status == ESP32_Status.OFFLINE and self.consecutive_errors >= 5:
                logger.warning(
                    f"[{self.device_id}] ⚠ Offline while producing measurements "
                    f"(buffer={len(self.buffer)}/{self.buffer_size}). Attempting reconnection..."
                )
    ...
    def _prepare_vl_data(self, vl_raw):
        ...
        status_matrix = vl_raw.get('status_matrix')
        if status_matrix is None:
            status_matrix = [[5] * 8 for _ in range(8)]
        if isinstance(status_matrix, np.ndarray):
    ...
    async def run(self, duration_seconds: Optional[int] = None):
        ...
        try:
            while True:
                ...
                # Tentative reconnexion si offline
                elif self.status == ESP32_Status.OFFLINE:
