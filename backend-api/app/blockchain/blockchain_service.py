...
class GaveursBlockchain:
    ...
    async def ajouter_evenement_sqal_quality(
        self,
        lot_id: int,
        gaveur_id: int,
        donnees_sqal: Dict
        ...
        """
        Ajoute un événement de contrôle qualité SQAL à la blockchain

        Args:
            lot_id: ID du lot
            gaveur_id: ID du gaveur
            donnees_sqal: Données complètes du contrôle SQAL (score, grade, sensors...)

        Returns:
            Bloc créé avec hash pour QR code
        """
        if not self.initialise:
            raise RuntimeError("Blockchain non initialisée")
        ...
        bloc = Block(
            ...
            canard_id=0,  # SQAL concerne le lot, pas un canard individuel
            gaveur_id=gaveur_id,
            donnees={
                "lot_id": lot_id,
                "sqal_score": donnees_sqal.get("sqal_score"),
                "sqal_grade": donnees_sqal.get("sqal_grade"),
                "sample_id": donnees_sqal.get("sample_id"),
                "vl53l8ch_data": donnees_sqal.get("vl53l8ch_data"),
                "as7341_data": donnees_sqal.get("as7341_data"),
                "compliance": donnees_sqal.get("compliance"),
                "control_timestamp": donnees_sqal.get("control_timestamp")
            },
            hash_precedent=self.chaine[-1].hash_actuel if self.chaine else "0"
        )
        ...
        logger.info(f"Bloc SQAL ajouté: lot {lot_id}, index {bloc.index}, hash {bloc.hash_actuel[:16]}...")
        return bloc

    async def ajouter_evenement_consumer_product(
        self,
        product_id: str,
        lot_id: int,
        gaveur_id: int,
        donnees_produit: Dict
    ) -> Block:
        """
        Ajoute un événement d'enregistrement produit consommateur à la blockchain

        Args:
            product_id: ID unique du produit
            lot_id: ID du lot
            gaveur_id: ID du gaveur
            donnees_produit: Données complètes du produit (SQAL, packaging, dates...)

        Returns:
            Bloc créé avec hash pour traçabilité
        """
        if not self.initialise:
            raise RuntimeError("Blockchain non initialisée")
        ...
        bloc = Block(
            ...
            donnees={
                ...
                "lot_id": lot_id,
                "site_code": donnees_produit.get("site_code"),
                "production_date": donnees_produit.get("production_date"),
                "packaging_date": donnees_produit.get("packaging_date"),
                "sqal_quality_score": donnees_produit.get("sqal_quality_score"),
                "sqal_grade": donnees_produit.get("sqal_grade"),
                "qr_code": donnees_produit.get("qr_code"),
                "certifications": donnees_produit.get("certifications", [])
            },
            hash_precedent=self.chaine[-1].hash_actuel if self.chaine else "0"
        )
        ...
        logger.info(f"Bloc produit consommateur ajouté: {product_id}, hash {bloc.hash_actuel[:16]}...")
        return bloc

    async def verifier_product_blockchain(self, blockchain_hash: str) -> Dict:
        """
        Vérifie l'authenticité d'un produit via son hash blockchain

        Args:
            blockchain_hash: Hash du bloc blockchain

        Returns:
            Dict avec résultat de vérification et données du bloc
        """
        try:
            async with self.db_pool.acquire() as conn:
                bloc = await conn.fetchrow(
                    """
                    SELECT
                        index,
                        timestamp,
                ...
                if not bloc:
                    return {
                        "valid": False,
                        "error": "Hash blockchain non trouvé",
                        "data": None
                    }

                # Vérifier l'intégrité du bloc
                donnees = json.loads(bloc["donnees"])

                # Recalculer le hash pour vérifier (utiliser les vraies valeurs stockées)
                # Note: Le hash original a été créé avec datetime.utcnow().isoformat() (sans timezone)
                # mais la DB stocke avec timezone. On retire la timezone pour correspondre.
                ts = bloc["timestamp"]
                timestamp_str = ts.replace(tzinfo=None).isoformat() if ts.tzinfo else ts.isoformat()

                contenu = {
                    "index": bloc["index"],
                    "timestamp": timestamp_str,
                    "type_evenement": bloc["type_evenement"],
                    "canard_id": bloc["canard_id"],
                    "gaveur_id": bloc["gaveur_id"],
                    "abattoir_id": bloc["abattoir_id"],
                    "donnees": donnees,
                    "hash_precedent": bloc["hash_precedent"]
                }

                contenu_str = json.dumps(contenu, sort_keys=True)
                hash_recalcule = hashlib.sha256(contenu_str.encode()).hexdigest()

                if hash_recalcule != bloc["hash_actuel"]:
                    return {
                        "valid": False,
                        "error": "Hash blockchain modifié - produit potentiellement frauduleux",
                        "data": None
                    }

                return {
                    "valid": True,
                    "timestamp": bloc["timestamp"].isoformat(),
                    "type_evenement": bloc["type_evenement"],
                    "data": donnees,
