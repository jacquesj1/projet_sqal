...
class ConsumerFeedbackService:
    ...
    async def init_pool(self, database_url: str, shared_pool: Optional[asyncpg.Pool] = None):
        ...
        Args:
            ...
            shared_pool: Pool partagé existant (optionnel, évite de créer un nouveau pool)
        """
        if shared_pool:
            self.pool = shared_pool
            logger.info("Consumer Feedback service using shared database pool")
        else:
    ...
    async def register_product_after_sqal(
        ...
        Intègre automatiquement la blockchain pour traçabilité
        ...
        try:
            async with self.pool.acquire() as conn:
                ...
                # Récupérer données produit pour blockchain
                ...
                # Ajouter événement blockchain produit consommateur
                if self.blockchain:
                    try:
                        # Parse certifications if it's a string
                        certs = product_data["certifications"] if product_data else []
                        ...
                        bloc = await self.blockchain.ajouter_evenement_consumer_product(
                            ...
                            donnees_produit={
                                "site_code": site_code,
                                "production_date": product_data["production_date"].isoformat() if product_data and product_data["production_date"] else None,
                                "packaging_date": product_data["packaging_date"].isoformat() if product_data and product_data["packaging_date"] else None,
                                "sqal_quality_score": float(product_data["sqal_quality_score"]) if product_data and product_data["sqal_quality_score"] else 0.0,
                                "sqal_grade": product_data["sqal_grade"] if product_data else "N/A",
                                "qr_code": qr_code,
                                "certifications": certs
                            }
                        )

                        # Mettre à jour le produit avec le hash blockchain
                        await self.link_product_to_blockchain(product_id, bloc.hash_actuel)

                        logger.info(
                            f"✅ Produit enregistré avec blockchain: {product_id} | "
                            f"QR: {qr_code[:30]}... | Blockchain: {bloc.hash_actuel[:16]}..."
                        )
                    except Exception as blockchain_error:
                        logger.warning(f"⚠️ Erreur blockchain (produit créé sans blockchain): {blockchain_error}")
                        import traceback
                        traceback.print_exc()
    ...
    async def scan_qr_code(self, qr_code: str) -> Optional[ProductTraceability]:
        ...
        try:
            async with self.pool.acquire() as conn:
                ...
                # Parse certifications (stored as JSON string in DB)
                ...
                traceability = ProductTraceability(
                    ...
                    quality_control_date=product["quality_control_date"] or product["production_date"],  # Fallback to production_date
