...
class AdvancedAnalyticsEngine:
    ...
    async def compare_genetiques(self, gaveur_id: Optional[int] = None) -> Dict:
        ...
        for record in records:
            # Convertir Decimal en float avant les opérations
            gain_moyen = float(record['gain_moyen']) if record['gain_moyen'] else 0
            gain_kg = gain_moyen / 1000.0 if gain_moyen > 0 else 0
            dose_kg = float(record['dose_moyenne_kg']) if record['dose_moyenne_kg'] else 0
            ic = dose_kg / gain_kg if gain_kg > 0 else 0

            comparaison.append({
                "genetique": record['genetique'],
                "nb_canards": record['nb_canards'],
                "gain_moyen_grammes": round(gain_moyen, 1),
                "gain_moyen_kg": round(gain_kg, 2),
                "dose_moyenne_kg": round(dose_kg, 2),
