...
async def get_quality_for_all_lots(request: Request):
    """
    Récupère les statistiques qualité SQAL agrégées par lot

    Utile pour analyses de corrélations globales (Euralis Analytics)

    Returns:
        Liste des lots avec métriques qualité moyennes:
        - score_qualite_moyen: Score fusion final moyen
        - tof_score_moyen: Score capteur ToF VL53L8CH moyen
        - spectral_score_moyen: Score capteur spectral AS7341 moyen
        - indice_fraicheur: Indice fraîcheur moyen (AS7341)
        - grade_majoritaire: Grade le plus fréquent (A+, A, B, C, REJECT)
        - nb_echantillons: Nombre d'échantillons SQAL pour ce lot
    """
    try:
        ...
        for row in rows:
            results.append({
                ...
                "score_qualite_moyen": float(row["score_qualite_moyen"]) if row["score_qualite_moyen"] else None,
                "grade_majoritaire": row["grade_majoritaire"],
                "tof_score_moyen": float(row["tof_score_moyen"]) if row["tof_score_moyen"] else None,
                "spectral_score_moyen": float(row["spectral_score_moyen"]) if row["spectral_score_moyen"] else None,
                "indice_fraicheur": float(row["indice_fraicheur"]) if row["indice_fraicheur"] else None,
                "indice_qualite_gras": float(row["indice_qualite_gras"]) if row["indice_qualite_gras"] else None,
                "indice_oxydation": float(row["indice_oxydation"]) if row["indice_oxydation"] else None,
            })

        logger.info(f"Récupéré statistiques SQAL pour {len(results)} lots")
        ...
        logger.error(f"Erreur récupération stats SQAL agrégées: {e}")
        raise HTTPException(status_code=500, detail=str(e))
