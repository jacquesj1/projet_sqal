...
import { Camera, Mic, AlertTriangle, TrendingUp, Clock, FileText, CheckCircle } from 'lucide-react';
import { voiceApi, canardApi, mlApi, gavageApi, VoiceParseResult, ParsedDocument } from '@/lib/api';
...
export default function SaisieRapideGavage() {
  ...
  const [lastVoiceCommand, setLastVoiceCommand] = useState<VoiceParseResult | null>(null);

  // OCR Modal
  const [showOCRModal, setShowOCRModal] = useState(false);
  const [lastScannedDocument, setLastScannedDocument] = useState<ParsedDocument | null>(null);
  ...
  const loadCanards = async () => {
    try {
      // TODO: ID gaveur dynamique (depuis context auth ou localStorage)
      const gaveurId = parseInt(localStorage.getItem('gaveur_id') || '1', 10);
      const data = await canardApi.getByGaveur(gaveurId) as CanardOption[];
      setCanards(data);
    } catch (error) {
  ...
  const calculerDoseTheorique = async () => {
    ...
    setLoading(true);
    try {
      const data = await mlApi.predictDoses(canardSelected) as {
  ...
  // ============================================
  // Reconnaissance Vocale avec Backend API
  // ============================================
  const startVoiceInput = useCallback(() => {
    ...
    recognition.onresult = async (event: any) => {
      ...
      setVoiceStatus(`Commande: "${transcript}"`);

      // Parser via backend API
      try {
        const result = await voiceApi.parse(transcript, {
          gaveur_id: 1, // TODO: ID dynamique
          canard_id: canardSelected || undefined,
        ...
        if (result.success && result.data) {
          ...
          setVoiceStatus(`✅ ${result.type}: ${result.data.valeur} ${result.data.unite || ''}`);
        } else {
          setVoiceStatus(`❌ Commande non reconnue: "${transcript}"`);
        }
      } catch (error) {
        console.error('Erreur voice API:', error);
        setVoiceStatus('❌ Erreur serveur');
      }
    };
  ...
  const applyVoiceCommand = (result: VoiceParseResult) => {
    if (!result.success || !result.data) return;

    const { type, data } = result;
    const valeur = data.valeur || 0;
    const sessionFromVoice = data.session || session;

    switch (type) {
      case 'dose':
        if (sessionFromVoice === 'matin') {
          setDoseMatin(valeur);
        } else {
          setDoseSoir(valeur);
        }
      ...
      case 'mortalite':
        // Ajouter aux remarques
        setRemarques((prev) =>
          prev ? `${prev}\nMortalité: ${valeur} canard(s)` : `Mortalité: ${valeur} canard(s)`
        );
        break;
    }
  };
  ...
  // ============================================
  const handleDocumentScanned = (document: ParsedDocument) => {
    setLastScannedDocument(document);

    // Traiter selon le type de document
    if (document.document_type === 'fiche_mortalite' && document.data.nombre_morts) {
      const msg = `Mortalité scannée: ${document.data.nombre_morts} canard(s)${
        document.data.lot_code ? ` - Lot ${document.data.lot_code}` : ''
      }${document.data.date ? ` - ${document.data.date}` : ''}`;
      setRemarques((prev) => (prev ? `${prev}\n${msg}` : msg));
    }

    if (document.document_type === 'bon_livraison') {
      alert(
        `📦 Bon de livraison détecté!\n\nDate: ${document.data.date_livraison || 'N/A'}\nQuantité: ${
          document.data.quantite_kg || 'N/A'
        } kg\nTotal: ${document.data.total_ht ? `${document.data.total_ht}€` : 'N/A'}`
      );
    }

    if (document.document_type === 'fiche_lot') {
      alert(
        `📋 Fiche lot détectée!\n\nCode: ${document.data.code_lot || 'N/A'}\nDate début: ${
          document.data.date_debut || 'N/A'
        }\nNb canards: ${document.data.nb_canards || 'N/A'}\nSouche: ${
          document.data.souche || 'N/A'
        }`
      );
    }
  };
  ...
  // ============================================
  // Vision par ordinateur (simulation)
  // ============================================
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-white rounded-lg shadow-lg">
      <h1 className="text-3xl font-bold mb-6 text-gray-800">🌽 Saisie Rapide de Gavage</h1>
      ...
      <div className="mb-6 p-4 bg-gradient-to-r from-purple-100 to-blue-100 rounded-xl">
        <div className="flex flex-wrap gap-3">
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 bg-white text-blue-700 hover:bg-blue-50 border border-blue-300 rounded-lg font-medium transition-all"
          >
            <FileText size={20} />
            Scanner Document
          </button>
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 bg-white text-green-700 hover:bg-green-50 border border-green-300 rounded-lg font-medium transition-all disabled:opacity-50"
          >
            <Camera size={20} />
            {cameraActive ? 'Détection...' : 'Détecter Poids'}
          </button>
        </div>
      ...
      {/* Sélection Canard */}
      <div className="mb-6">
        <label className="block text-sm font-medium text-gray-700 mb-2">Canard</label>
        <select
        ...
        <div className="flex gap-4">
          <button
            ...
            className={`flex-1 p-3 rounded-lg font-medium ${
              session === 'matin' ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700'
            }`}
          >
            🌅 Matin
          </button>
          <button
            ...
            className={`flex-1 p-3 rounded-lg font-medium ${
              session === 'soir' ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700'
            }`}
          >
            🌙 Soir
          </button>
        ...
        <div className="mb-6 p-4 bg-gradient-to-r from-purple-100 to-blue-100 rounded-lg border border-purple-300">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <p className="text-2xl font-bold text-purple-700">{doseTheorique.dose_matin}g</p>
            </div>
            <div>
              <p className="text-sm text-gray-600">Soir</p>
              <p className="text-2xl font-bold text-purple-700">{doseTheorique.dose_soir}g</p>
            </div>
          </div>
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">Dose Matin (g)</label>
          <input
            type="number"
            value={doseMatin}
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">Dose Soir (g)</label>
          <input
            type="number"
            value={doseSoir}
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">Poids Matin (g)</label>
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">Poids Soir (g)</label>
          <div className="flex gap-2">
            <input
              type="number"
              value={poidsSoir}
      ...
      <div className="mb-6">
        ...
        {isListening && (
          <p className="text-sm text-red-600 mt-1 animate-pulse">🎤 Écoute en cours...</p>
        )}
      </div>

      {/* Dernière commande vocale reconnue */}
      {lastVoiceCommand && lastVoiceCommand.success && (
        <div className="mb-6 p-3 bg-green-50 border border-green-200 rounded-lg">
          <div className="flex items-center gap-2 text-green-800">
            <CheckCircle size={16} />
            <span className="text-sm font-medium">
              Dernière commande: {lastVoiceCommand.type} = {lastVoiceCommand.data.valeur}
              {lastVoiceCommand.data.unite}
              {lastVoiceCommand.data.session && ` (${lastVoiceCommand.data.session})`}
            </span>
          </div>
        </div>
      )}
        ...
        <div className="mt-6 p-4 bg-gray-50 rounded-lg">
          ...
          <div className="grid grid-cols-3 gap-4 text-center">
            <div>
              ...
              <p className="text-xl font-bold text-green-600">+{poidsSoir - poidsMatin || 0}g</p>
            </div>
            <div>
