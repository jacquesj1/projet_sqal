...
import { alerteApi, analyticsApi } from '@/lib/api';
import { DEFAULT_GAVEUR_ID } from '@/lib/constants';
import { formatDateShort, getNiveauColor, getNiveauIcon, getScoreColor } from '@/lib/utils';
import type { Alerte, AlerteDashboard, PerformanceMetrics, CourbesPrediction } from '@/lib/types';

export default function AnalyticsPage() {
  const [alertes, setAlertes] = useState<Alerte[]>([]);
  const [dashboardAlertes, setDashboardAlertes] = useState<AlerteDashboard | null>(null);
  ...
  const [comparaisonGenetiques, setComparaisonGenetiques] = useState<{ comparaisons: ComparaisonItem[] } | null>(null);
  const [rapportHebdo, setRapportHebdo] = useState<RapportHebdo | null>(null);
  ...
  const loadDashboardAlertes = async () => {
    try {
      const data = await alerteApi.getDashboard(DEFAULT_GAVEUR_ID) as AlerteDashboard;
  ...
  const loadAlertes = async () => {
    try {
      const data = await alerteApi.getByGaveur(DEFAULT_GAVEUR_ID, false) as Alerte[];
      setAlertes(data);
    } catch (error) {
  ...
  const loadMetrics = async () => {
    ...
    try {
      const data = await analyticsApi.getMetrics(canardSelected) as PerformanceMetrics;
      setMetrics(data);
    } catch (error) {
  ...
  const loadPredictions = async () => {
    if (!canardSelected) return;
    try {
      const data = await analyticsApi.getPredictions(canardSelected, 7) as CourbesPrediction;
      setPredictions(data);
    } catch (error) {
  ...
  const loadComparaisonGenetiques = async () => {
    try {
      const data = await analyticsApi.compareGenetiques(DEFAULT_GAVEUR_ID) as { comparaisons: ComparaisonItem[] };
  ...
  const loadRapportHebdo = async () => {
    try {
      const data = await analyticsApi.getWeeklyReport(DEFAULT_GAVEUR_ID) as RapportHebdo;
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {selectedTab === 'predictions' && (
          <div className="space-y-6">
            <div className="bg-white rounded-lg shadow-lg p-6">
              <label className="block text-sm font-medium text-gray-700 mb-2">
                Sélectionner un canard pour les prédictions
              </label>
              <input
                type="number"
                placeholder="ID du canard"
            ...
            {predictions && predictions.previsions && (
              <div className="bg-white rounded-lg shadow-lg p-6">
                <h2 className="text-2xl font-bold mb-6 flex items-center gap-2">
                ...
                <ResponsiveContainer width="100%" height={400}>
                  <AreaChart data={predictions.previsions}>
                    ...
                    <YAxis />
                    <Tooltip formatter={(value: number) => [`${value.toFixed(0)}g`, 'Poids']} />
                    <Legend />
                    <Area
                      type="monotone"
                      dataKey="poids_max"
                ...
                <div className="mt-6 p-4 bg-blue-50 rounded-lg">
                  <p className="text-sm text-blue-900">
                    <strong>Intervalle de confiance 95%</strong> : Les zones ombrées représentent la
                    plage probable du poids futur avec une confiance de 95%.
                  </p>
                </div>
              </div>
            )}
            ...
            <div className="bg-white rounded-lg shadow-lg p-6">
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  ...
                  <tbody>
                    {comparaisonGenetiques.comparaisons.map((comp, idx) => (
        ...
        {rapportHebdo && (
          <div className="mt-8 bg-gradient-to-br from-indigo-50 to-purple-50 rounded-lg shadow-lg p-6 border-2 border-indigo-300">
            <h2 className="text-2xl font-bold mb-6 text-indigo-900">📈 Rapport Hebdomadaire</h2>
            ...
            {rapportHebdo.top_performers && rapportHebdo.top_performers.length > 0 && (
              <div className="mt-6">
                ...
                <div className="space-y-2">
                  {rapportHebdo.top_performers.map((perf, idx) => (
                    <div key={idx} className="flex items-center justify-between bg-white p-3 rounded-lg">
                      <span className="font-medium">
                        {idx + 1}. {perf.numero}
                      </span>
                      <span className="font-bold text-green-600">{perf.gain_moyen.toFixed(0)}g/jour</span>
                    </div>
                  ))}
                </div>
              </div>
